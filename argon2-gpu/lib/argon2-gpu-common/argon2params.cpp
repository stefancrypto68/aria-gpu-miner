...
/// EXFER mining extension: H0 with a runtime-provided salt.
/// Mirrors initialHash() but substitutes the salt at runtime, so a single
/// Argon2Params instance can drive a batch of hashes that each carry their
/// own salt (one per nonce).
static void initialHashWithSalt(
        void *out,
        std::uint32_t lanes, std::uint32_t outLen,
        std::uint32_t m_cost, std::uint32_t t_cost,
        Version version, Type type,
        const void *pwd, std::size_t pwdLen,
        const void *salt, std::size_t saltLen,
        const void *secret, std::size_t secretLen,
        const void *ad, std::size_t adLen)
{
    ...
    store32(value, m_cost);      blake.update(value, sizeof(value));
    store32(value, t_cost);      blake.update(value, sizeof(value));
    store32(value, (std::uint32_t)version); blake.update(value, sizeof(value));
    store32(value, (std::uint32_t)type);    blake.update(value, sizeof(value));
    store32(value, (std::uint32_t)pwdLen);  blake.update(value, sizeof(value));
    blake.update(pwd, pwdLen);
    store32(value, (std::uint32_t)saltLen); blake.update(value, sizeof(value));
    blake.update(salt, saltLen);
    store32(value, (std::uint32_t)secretLen); blake.update(value, sizeof(value));
    blake.update(secret, secretLen);
    store32(value, (std::uint32_t)adLen); blake.update(value, sizeof(value));
    blake.update(ad, adLen);

    blake.final(out, ARGON2_PREHASH_DIGEST_LENGTH);
}

void Argon2Params::fillFirstBlocksWithSalt(
        void *memory,
        const void *pwd, std::size_t pwdLen,
        const void *altSalt, std::size_t altSaltLen,
        Type type, Version version) const
{
    std::uint8_t initHash[ARGON2_PREHASH_SEED_LENGTH];
    initialHashWithSalt(
            initHash, lanes, outLen, m_cost, t_cost, version, type,
            pwd, pwdLen, altSalt, altSaltLen,
