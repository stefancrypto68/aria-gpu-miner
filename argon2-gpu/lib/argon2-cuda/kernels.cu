...
__device__ void load_block_xor(struct block_th *dst, const struct block_g *src,
                               uint32_t thread)
{
    // load_block_xor is called only on `mem_ref` (the reference block) inside
    // argon2_core — that block is read-only within the current step. We can
    // safely use __ldg (read-only L1/TEX cache) which on Ampere+ also gives
    // the scheduler more room to overlap with the prev-block compute.
    // Compatible from SM_35.
#if __CUDA_ARCH__ >= 350
    dst->a ^= __ldg(&src->data[0 * THREADS_PER_LANE + thread]);
    dst->b ^= __ldg(&src->data[1 * THREADS_PER_LANE + thread]);
    dst->c ^= __ldg(&src->data[2 * THREADS_PER_LANE + thread]);
    dst->d ^= __ldg(&src->data[3 * THREADS_PER_LANE + thread]);
#else
    dst->a ^= src->data[0 * THREADS_PER_LANE + thread];
    dst->b ^= src->data[1 * THREADS_PER_LANE + thread];
    dst->c ^= src->data[2 * THREADS_PER_LANE + thread];
