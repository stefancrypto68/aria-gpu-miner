...
public:
    ...
    /// EXFER mining extension: per-hash salt override.
    /// Same as fillFirstBlocks but uses the provided salt instead of the
    /// one stored in this Params instance. Lets a single ProcessingUnit
    /// batch hashes with distinct salts (one per nonce).
    void fillFirstBlocksWithSalt(
            void *memory,
